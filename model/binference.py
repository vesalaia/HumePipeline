...
def load_line_model(opts, device):
    if opts.line_detection_model == "maskr-cnn":
...
def predict_image(opts, model, imagepath):
...
def get_regions(opts, imagepath, model, OCRclasses):
    ...
    outputs, o_rows, o_cols = predict_image(opts, model, imagepath)
...
#       fix it by re-training the model
        ...
        resized_logits = nn.functional.interpolate(logits,
                ...
                mode='bilinear',
                align_corners=False)
        seg  = resized_logits.detach().squeeze().numpy()
